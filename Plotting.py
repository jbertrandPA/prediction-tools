...
def plot_df(df, x_label, y_label, title, show_std = False):
    ...
    if show_std:
        update_plot_df_mean_std(plt, df, df[x_label], y_label)
    plt.xticks(rotation=60)

def update_plot_df_mean_std(plt, df, x_range, y_label):
    m = df[y_label].mean()
    s = df[y_label].std()
    df["mean"] = m
    df["mean+std"] = s + m
    df["mean-std"] = m - s
    plt.plot(x_range, df["mean"], 'r--')
    plt.plot(x_range, df["mean+std"], 'g--')
    plt.plot(x_range, df["mean-std"], 'g--')
...
def plot_bar_df(df, x_label, y_label, title, show_std = False):
    plt.figure(figsize=(15, 10))
    plt.title(title, size=20)
    plt.bar(range(0,len(df[y_label])), df[y_label], tick_label = df[x_label])
    if show_std:
        update_plot_df_mean_std(plt, df, range(0,len(df[y_label])), y_label)
    plt.xticks(rotation=60)